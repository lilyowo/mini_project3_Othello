...
int minimax(int depth, OthelloBoard& board, int alpha, int beta)
{
	...
	if (depth % 2 == 1) {//
		...
		for (it = child.begin(); it != child.end(); it++) {
			...
			int m = minimax(depth + 1, next_board, alpha, beta) + good_pos((*it).x, (*it).y);
...
void write_valid_spot(std::ofstream& fout) {
	...
	for (it = next_valid_spots.begin(); it != next_valid_spots.end(); it++) {
		OthelloBoard next_board = Board;
		next_board.put_disc(*it);  //≤ƒ§@ºh
		int m = 0;
		if (next_board.done == 1)
			m = next_board.count_value() + good_pos((*it).x, (*it).y);
		else m = minimax(2, next_board, alpha, beta) + good_pos((*it).x, (*it).y);
		if (m > bp) {
			bp = m;
			best_point = *it;
			alpha = m;
